...
async def calculate_route(route: RouteRequest):
    try:
        ...
        if route.smooth:
            ...
            # Для резкой траектории добавляем промежуточные точки с динамическим количеством
            sharp_points = []

            # Функция расчета расстояния между точками
            def calculate_distance(p1, p2):
                return math.sqrt((p2.lng - p1.lng) ** 2 + (p2.lat - p1.lat) ** 2 + (p2.alt - p1.alt) ** 2)
            ...
            for i in range(len(sorted_points) - 1):
                ...
                # Динамически определяем количество промежуточных точек:
                # - минимум 5 точек на отрезок
                # - дополнительно 1 точка на каждые 0.001 градуса (примерно 100 метров)
                num_points = max(5, int(distance * 1000))
            ...
            sharp_points.append({
                "lat": sorted_points[-1].lat,
                "lng": sorted_points[-1].lng,
                "alt": sorted_points[-1].alt
            })

            return {"status": "success", "points": sharp_points}
